...
**Phase:** Day Zero — vision, SPEC v0.1, defaults resolved, Go skeleton in place. **Identity, memory, activity-log, RPC server, three provider adapters (Claude + OpenAI + Ollama-chat), the Ollama embedder, and application-level row encryption (SPEC §5.1) all landed. Mediated mode is real; the cosine retrieval path is live when Ollama is running; the Provider interface is exercised against three wire formats (Anthropic Messages, OpenAI Responses, Ollama /api/chat); memory rows are AES-256-GCM-encrypted at rest under an HKDF-derived, identity-bound key with no CGO and no SQLCipher dependency.** Every primitive SPEC v0.1 demands is now in tree; remaining v0.1 work is the CLI surface, the demo video, and the NLnet application.

**Repository:** https://github.com/regitxx/Daimon.git
**Build status:** `make build` → `bin/daimond` (~15.2 MB). 146/146 tests pass in ~10s (9 identity + 30 memory + 11 activity + 32 server + 12 provider + 10 claude adapter + 13 openai adapter + 17 ollama-chat adapter + 12 ollama embedder), race-clean. Demo run generates a `did:key:z6Mk…` DID, **probes a local Ollama server and falls back to `NullEmbedder` per SPEC §11 if absent**, opens a SQLite memory store **whose `content`/`metadata`/`source` columns are AES-256-GCM-encrypted with row-id-bound AAD under a key derived from the identity's Ed25519 seed via HKDF-SHA256**, opens a BLAKE3-chained activity log, writes three signed memories (each emitting a signed activity entry), runs a memory search and labels the top hit's retrieval path (cosine vs substring fallback), exports a signed memory document, re-imports it into a fresh-identity store **(receiver re-encrypts under its own derived key)**, verifies the activity chain end-to-end, builds a provider registry — Claude registers iff `ANTHROPIC_API_KEY` is set, OpenAI registers iff `OPENAI_API_KEY` is set, **Ollama-chat registers iff a probe of `/api/tags` on `OLLAMA_HOST` (default `localhost:11434`) succeeds, and its model list reflects what is actually pulled on the local server** — then stands up the JSON-RPC 2.0 server on a Unix socket and self-calls `daimon.identity.get` + `daimon.provider.list`. Every signature checks out, the RPC roundtrips succeed, and each registered adapter advertises its model list with `configured=true`.
...
15. **CLI (`cmd/daimon` — wraps RPC for terminal use; SPEC §11 surface: `init / unlock / memory / provider / chat`)** ← *next session candidate*
